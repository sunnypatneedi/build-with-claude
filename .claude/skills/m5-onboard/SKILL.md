...
- **`esptool>=5` is not yet supported.** `requirements.txt` pins `esptool>=4.11,<5` and that upper bound is intentional. `flash.py` uses the underscore-form CLI options (`--before no_reset`, `--after watchdog_reset`, `write_flash`) that 5.x deprecates *and* changes the connect-time sync behavior of `--before no_reset --no-stub` against the Cardputer-Adv USB-JTAG bootloader. Symptom on 5.x: the post-button-dance flash hangs at "Connecting......" then exits with `Failed to connect to Espressif device: No serial data received.` Hit this on 2026-05-07 — esptool 5.2.0 was the freshly-installed version on a machine where 4.x had never been pinned. Lift the upper bound only after `flash.py` migrates to the hyphenated option names and is reverified end-to-end on Cardputer-Adv.
- **USB-C cable orientation matters on Cardputer-Adv.** The board's USB-C port enumerates only one orientation — flip it 180° and the device disappears from `ls /dev/cu.usbmodem*` even though it still gets power and the LCD lights up. Same physical cable, same machine, just rotated. If `detect.py` reports no Espressif port despite the device being plugged in *and the screen on*, before assuming a hardware fault: ask the user to flip the cable orientation. We hit this on 2026-05-07 after a successful flash where the post-watchdog-reset USB simply didn't reappear; flipping the cable resolved it instantly. (USB-A-to-USB-C cables are unaffected — the orientation issue is C-to-C only.)
- **Watchdog reset can leave the USB-CDC interface silent.** After `flash.py` finishes with `--after watchdog_reset`, the device reboots into UIFlow and *usually* re-enumerates as `/dev/cu.usbmodemXXX`. Occasionally it doesn't — `_write_boot_py` then opens the pre-flash port name and gets `[Errno 2] No such file or directory`. Recovery: ask the user to either tap BtnRST once or unplug-and-replug the USB cable. Once the port is back, run `install_apps.py --port <PORT> --src buddy` separately rather than re-running the full `onboard.py` (no need to re-flash UIFlow that just succeeded).
